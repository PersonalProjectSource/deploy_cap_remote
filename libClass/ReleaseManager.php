...
//        echo('@@@@@@@@@@@ln -s '.self::API_ROOT_PATH.''.$sPathReversed.' '.self::VHOST_PATH.'@@@@@@@@'."\n");
	...
	echo self::VHOST_PROJECT_PATH;
	
	
	echo('cd  /var/www/ && sudo rm bitume'."\n" );
        echo('cd /var/www/ && sudo ln -s /var/www/deploy/'.$sPathReversed.' '.self::VHOST_PROJECT_PATH.''.self::PROJECT_NAME."\n");
	system('cd  /var/www/ && sudo rm bitume');
        system('cd /var/www/ && sudo ln -s /var/www/deploy/'.$sPathReversed.' bitum3');
	//                      sudo ln -s /var/www/deploy/sourceRelease/1438173284 bitume2    
	//system('mv /var/www/deploy/'.$sPathReversed.' /var/www/bitume');
